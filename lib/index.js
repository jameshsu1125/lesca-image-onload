...
var e = /* @__PURE__ */ function(e) {
	...
	hideBeforeLoaded;
	constructor(e, t) {
		this.target = e, this.hideBeforeLoaded = t === void 0 ? !0 : t, this.index = 0, this.result = [];
	}
	load(n = t) {
		...
		let { onUpdate: r, onStart: i } = {
			...t,
			...n
		}, a = this.getStyle(this.target, "display") === "flex" ? "flex" : "block";
		this.hideBeforeLoaded && (this.target.style.display = "none");
		var o = Array.from(this.target.querySelectorAll("*"));
		[this.target, ...o].forEach((t, n) => {
		...
		let s = ({ resolve: t = (e) => console.log(e), reject: n = (e) => console.log(e) }) => {
			if (this.target === null) {
				n({
					total: this.result.length,
					loaded: 0
			...
			if (this.result.length === 0) {
				this.hideBeforeLoaded && (this.target.style.display = a), t({
					total: 0,
					loaded: 0
				});
				return;
			}
			let i = this.result[this.index], o = this.result.length, { url: c, index: l } = i;
			i.status = e.loading;
			let u = new Image();
			u.onload = () => {
				i.status = e.loaded;
				let u = this.result.filter((t) => t.status === e.loaded).length;
				o === u ? (this.hideBeforeLoaded && this.target && (this.target.style.display = a), requestAnimationFrame(() => t({
					url: c,
					total: o,
					loaded: u,
					index: l
