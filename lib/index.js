...
var ImagePreloader = /** @class */ (function () {
    ...
    ImagePreloader.prototype.load = function (target, options) {
        ...
        elements.forEach(function (e, index) {
            ...
            var backgroundImage = _this.getStyle(e, 'background-image').replace(/url\((['"])?(.*?)\1\)/gi, '$2');
            var maskImage = _this.getStyle(e, '-webkit-mask-image').replace(/url\((['"])?(.*?)\1\)/gi, '$2');
            var status = type_1.Status.unload;
            if (tag === 'IMG' && src) {
        ...
        var loadImage = function (_a) {
            ...
            data.status = type_1.Status.loading;
            var image = new Image();
            image.onload = function () {
                data.status = type_1.Status.loaded;
                var loaded = _this.result.filter(function (e) { return e.status === type_1.Status.loaded; }).length;
                if (total === loaded) {
                    if (hideBeforeLoaded)
                        target.style.display = display;
